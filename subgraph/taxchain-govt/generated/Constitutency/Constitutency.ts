...
export class Constitutency extends ethereum.SmartContract {
  static bind(address: Address): Constitutency {
    return new Constitutency("Constitutency", address);
  }

  constituency(): Address {
    let result = super.call("constituency", "constituency():(address)", []);

    return result[0].toAddress();
  }

  try_constituency(): ethereum.CallResult<Address> {
    let result = super.tryCall("constituency", "constituency():(address)", []);
    if (result.reverted) {
...
export class SetConstituencyAddressCall extends ethereum.Call {
  get inputs(): SetConstituencyAddressCall__Inputs {
    return new SetConstituencyAddressCall__Inputs(this);
  }

  get outputs(): SetConstituencyAddressCall__Outputs {
    return new SetConstituencyAddressCall__Outputs(this);
  }
}

export class SetConstituencyAddressCall__Inputs {
  _call: SetConstituencyAddressCall;

  constructor(call: SetConstituencyAddressCall) {
    this._call = call;
  }

  get _constAddress(): Address {
    return this._call.inputValues[0].value.toAddress();
  }
}

export class SetConstituencyAddressCall__Outputs {
  _call: SetConstituencyAddressCall;
